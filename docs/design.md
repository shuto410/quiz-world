...
- UIは明るいライトテーマを既定とし、生成りの背景、濃いブラウンの文字、アプリコットの主操作色で統一する。リンクや小さな強調文字は読みやすい濃いテラコッタを使う。
- 大会画面はデスクトップで上部に大会情報と退出、主領域に進行操作、右に参加者一覧を配置する。招待情報は上部から開閉できる。装飾コピーや実際に切り替えられないナビゲーションは置かない。スマートフォンでは1列にし、参加者一覧は開閉可能にする。
...
- 参加者プレイ画面: 参加者一覧、早押し順、スコアを中心に表示。中央の操作エリアに大きな早押しボタン、進行中は回答入力欄を常設
- ホスト進行画面: 参加者一覧、接続状態、スコア、早押し順を中心に表示。中央に回答内容と状態に応じた判定操作を表示
