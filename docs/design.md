<!-- Authoritative product and system specification; implementation changes must follow this document. -->
...
フェーズ0では、通常の利用でも起きる切断・再起動と、低コストで防げる状態分裂だけを対策する。

- Socket再接続のたびに再joinし、join成功と最新の `room:state` を受け取るまで操作を無効にする
- 切断した参加者をオフラインにし、ホスト切断時はゲームを `paused` にする
- サーバー再起動時は `RoomSnapshots` からスコアと進行状態を復元する
- 終了済み大会でも、既存参加者は同じ席へ再接続して結果を見られるようにする
- ECS更新時も旧新タスクを同時に動かさず、常に単一の権威プロセスだけを稼働させる
- `room:close` は `Tournaments.status = closed` を確認してからルームの所有権を手放す

一方、意図的な攻撃への対策や大規模化の仕込みは、身内だけが参加するフェーズ0では増やさない。受容するリスクは「フェーズ0で受容する既知リスク」にまとめる。
...
- 再接続を `participantId` 単体の申告から、サーバー発行の秘密の再接続トークン検証へ切り替える
- ホスト引き継ぎに待機時間または引き継ぎ用秘密情報を追加し、任意参加者による即時取得を防ぐ
- CloudWatch メトリクスを整備する: 同時ルーム数、同時接続数、早押し受信間隔
- CloudFront固有のオリジンヘッダーをALBで検証し、CloudFrontからALBへの通信をHTTPSにする
...
- `desiredCount: 1` だけではローリング更新中に旧新2タスクが並存できるため、CDKで `minHealthyPercent: 0`、`maxHealthyPercent: 100` を指定して旧タスク停止後に新タスクを起動する
...
- クライアントはSocket再接続のたびにjoinイベントを再送し、サーバー上のSocketセッションとルーム参加を作り直す
...
- `Host`: 大会作成、ルール設定、正誤判定、進行操作、大会終了、ルームクローズ
...
- 途中参加と途中退出を許可する。途中参加者はスコア0・正解数0・誤答数0から始まる
- 退出者のスコアと表示名はルーム内に残す
- 同じ大会内で表示名の重複は許可しない
...
flowchart LR
  ...
  result --> answering2["answering: next responder"]
  answering2 --> result
  result --> idle
...
- ホストは「正解」「不正解」を押して `judge:submit` を送り、サーバーは設定済みルールで得点と○・×数を更新して `result` に移る
- 判定後の進行は `game:reset`（次の問題へ）と、不正解時の `game:next-responder`（次の回答者へ）に分離する。どちらも得点を変更しない
...
- 押した順には、1着を「1着」、2着以降を1着との差「+0.34秒」で表示する。差はサーバーの受信時刻 `receivedAt` の差で、通信の遅れも含む（押した速さそのものではない）。ホストの大きい表示では 0〜2.5 秒を横棒でも示す。押すまでの絶対時間は表示しない。差は表示専用で、順番や回答権の判定には使わない
...
- ホストはルームの「試合ルール」で、得点制または n○m× を選ぶ。既定は得点制（正解 +1 点、不正解 0 点）。7○3×プリセットを用意する
- 得点制では正解時・不正解時の増減点をそれぞれ -999〜999 の整数で設定する。問題ごとの配点入力は行わない
- n○m×では勝ち抜けに必要な正解数と失格になる誤答数をそれぞれ 1〜99 で設定する。7○3×は7正解で勝ち抜け、3誤答で失格。正解は +1 点、不正解は 0 点で、○数と×数を独立して記録する
- ルールの変更は最初の判定前かつ `idle` のホストだけが行える。判定済みの大会には途中から適用しない。ルール、点数、○・×数はスナップショットから復元する
- 正誤はホストの手動判定のみ。「正解」「不正解」を押すと直ちにサーバーへ送り、設定済みの増減点と○・×数を同期的に更新して全員へ配信する。別の確定ボタンは設けない
- 判定は必ず `result` に移り、全員に正誤、得点変動、○・×数、勝ち抜け・失格の表示を出す。未送信の口頭回答も判定できる
- 判定後にホストが「次の問題へ」または不正解時の「次の回答者へ」を選ぶ。進行操作は得点を変更しない。自動進行は追加しない
- n○m×の勝ち抜け・失格条件は共有の純粋関数で定義し、サーバーが判定時と早押し受付時に適用する。クライアントは同じ条件を表示とボタン無効化に使う。勝ち抜け・失格した席の早押しはサーバーでも拒否する
- 送信済みテキスト回答は `result` の間だけ全員へ配信する。次の回答者へ移ると破棄する。直前の正誤通知は次の回答中にも残し、新しい判定または次の問題で置き換える
- 得点制の順位は従来どおり得点順。n○m×は勝ち抜け、競技中、失格の順で、各区分は○数降順・×数昇順。同条件は同順位。最終結果は達成者全員を「勝ち抜け」と表示し、未達成者や失格者を優勝扱いしない。勝ち抜け順や人数による自動終了は今回の対象外
- 参考: [Nagaya Quiz Arena 3](https://penpenpng.com/nqa3/room4) の正解数・誤答数の独立表示とルール確認UI。ほかの特殊形式は追加しない

### 画面構成

- UIは仲間と囲むクイズの進行卓とスコアボードを軸に、デザインシステム「クイズバトル」のキャラメル&ハニー配色で統一する。暖色の中間トーン（`surface-page`）を背景にし、パネル・カード・行をその順に背景より明るく重ねる。文字は濃い茶（`ink`）を基本とし、白は早押しボタンの赤の上と24px以上のロゴ・大見出しだけに使う。見出し・数字・ボタンは Dela Gothic One、本文とラベルは M PLUS Rounded 1c を使う。押せるものは下側の濃い色の厚みで表現し、押すと沈む。ゲーム用のメインスペースは明るいパネルで大会情報・スコアボードから明確に区切り、その中にモード名、表示領域、操作領域、補足情報をまとめる。内部はカードと行の明るさの段差で整理する。回答者・回答内容・得点を大きく読みやすくし、待機案内は控えめにする。赤（`buzzer`）は早押しと主操作、アクセント色は回答中、金色は1位と自分の席に使う。1画面に赤いボタンを2つ並べず、2つ目以降は枠線だけの控えめなボタンにする。正誤は緑と桃色の面で区別し、記号と文字も併記する。
- 大会画面はデスクトップで上部に大会情報と退出、主領域に進行操作、右に参加者一覧を配置する。上部ヘッダーは背景色の上に、左に早押しボタンを簡略化したアイコン付きの白いロゴ、中央にモード名と大会名のチップ、右にホストのみルームコード（開くと招待URLのコピー）、試合ルールを開く歯車ボタン、自分のアバター・名前・順位と得点（開くと名前変更と退出）を載せる。ホスト画面は左上に判定バー（回答者と正誤ボタン）、左下に押した順、右に得点ランキング。参加者画面は左に早押しボタンのパネル、中央に押した順、右に得点ランキングの3列とする。スマートフォンでは押した順と得点ランキングを「着順・得点」のタブで切り替える。招待・参加情報の操作をゲーム領域から穏やかに区切り、明るいメインスペースを主役にする。招待情報は上部の「招待する」に集約し、参加者が0人でも主領域には重複表示しない。装飾コピーや実際に切り替えられないナビゲーションは置かない。スマートフォンでは1列にする。
- 正誤ボタン、フォーカス、無効状態を視覚的に区別する。

- 大会作成画面: 大会名と最大参加人数を入力し、招待コードと招待URLを確認する
- 招待コード入力画面: コードを直接入力して参加画面へ進む
- 参加者名前入力画面: 大会名を表示し、表示名だけを入力する
- 参加者プレイ画面: 参加者一覧、早押し順、スコアを中心に表示。中央の安定した操作領域に早押しボタンを置き、回答権を得たら回答入力へ切り替える。再接続やホスト切断による一時停止中は回答欄と入力中の文字を保持したまま操作を無効化する
- ホスト進行画面: デスクトップでは左に回答者と回答、右に判定操作を配置し、通常の短い回答はスクロールせずに判定できるようにする。狭い画面では縦に並べる。手順説明は開閉式にする
- 最終結果画面: 得点制は優勝者を強調表示し、同点1位は全員を優勝者とする。n○m×は勝ち抜け者と○・×数、失格を表示する。ホスト席は順位に含めない

参加者画面は状態が変わっても基本レイアウトを維持する。ホスト画面の操作ボタンは状態ごとに絞る。

参加者一覧は設定された試合ルールの順位で並べるスコアボードとし、同条件は同順位、次順位は人数分飛ばす。名前と等幅の得点を揃え、名前の横に小さな色付きの駒を置く。駒の色は参加者IDから決め、順位・表示名・接続状態が変わっても維持する。早押し順にも同じ駒を使う。色は装飾的な目印であり、一意な識別や回答権の判別を色だけに依存させない。現在の回答者には「回答中」の文字を表示し、駒自体を同系色でわずかに濃くして内側の縁を強調する。自分の席は「あなた」で区別する。ホストは得点を表示せず別枠にする。名前変更と退出は参加情報から開き、招待は独立して見つけやすくする。大会名と作成時の招待情報は大会ID別にブラウザに保存し、参加者も参加時に取得した大会名を再表示できる。既存のsessionStorage内の招待情報も読み取る。

待機中のホスト画面は参加者が0人なら「参加者を待っています」、参加後は「早押しを待っています」と表示する。回答者名は回答中に強調し、結果では正誤と得点変動を主役にする。早押しボタンは指で押せる大きさの、真上から少し斜めに見た円筒（上面が赤い楕円、側面が濃い赤）とし、押下時に上面が側面へ沈み込むことで反応を伝える。参加者の操作領域は回答入力に切り替わっても位置を保つ。

動きは回答権の通知と得点変化の短い強調に限定し、早押しの操作を待たせない。動きを減らす設定では位置・拡大縮小のアニメーションを行わない。

- `idle`: スコア確認、大会終了
- `answering`: 正解・不正解の即時判定
- `result`: `game:reset`（次の問題へ）、不正解時の `game:next-responder`、大会終了
- `paused`: 引き継ぎ・再接続状態の表示のみ
- `finished`: 最終結果表示、ルームクローズ

#### ゲームモードを追加するための共通UI

大会の接続と運営はゲームモードから分離する。`RoomSession` が接続表示、役割と表示名、招待、参加者とスコア、名前変更、退出、ホスト引き継ぎ、最終結果とルーム終了をまとめる。Socket は従来どおりページが保持し、役割の変更で再接続しない。`RoomFrame` はその配置だけを担当する。

`GamePanel` はモード名、現在の状態、次に行うこと、中央の表示領域、操作領域を受け取る表示専用の枠とする。視覚上も一つの独立したゲームスペースとして外枠と背景を持ち、待機・回答・結果の各状態でもその境界を維持する。表示領域は画像などを渡せる余白を確保し、ホストの操作領域は同じ枠の中で仕切る。早押し固有の回答権・早押し順・判定は `modes/buzzer` に置き、大会運営部分を複製しない。早押しでは参加者の回答番を強調し、受付済み・判定待ち・一時停止・再接続を区別する。操作可否は従来のサーバー状態と接続状態から導出する。結果表示と大会終了は異なる状態として扱う。

中央領域は固定の早押しボタンを前提にせず、将来は複数画像などのコンテンツを渡せる。ホストは表示と操作を左右に、参加者は上下に配置できる。モバイルでは中央領域、操作、補足情報、参加者の順に1列で表示し、横幅を固定しない。画像モードのルール、素材の管理・配信、選択UI、サーバーのモード識別は今後の設計対象であり、今回は追加しない。現在のモードは「早押しクイズ」のみとし、未実装モードの選択肢は表示しない。
...
/**
 * Latest judgement retained until the next question.
 */
...
- 優勝者: 得点制ではホスト席を除く最高スコア。n○m×では設定された○数に達した席を勝ち抜けとして表示する
- ランキング順位: 上記ルールに従って並べる。勝ち抜け・失格フラグや順位は状態に保存しない
- 早押し順位: `buzzOrder` の配列インデックスから計算する
- `isHost`: `hostId === participant.id` で計算する
- `hasBuzzed`: `buzzOrder` に参加者IDが含まれるかで計算する
- `isCurrentResponder`: `answering` 中かつ `currentResponderId` との一致で計算する。`result` 中のIDは判定済みの履歴であり回答権ではない
...
- `judge:submit`: `lastResult` をセットして `result` へ。`currentBuzzSession`、`buzzOrder`、`currentResponderId`、`currentSubmittedAnswer` は結果確認と次候補への進行のため保持する。`result` 中の早押し・回答送信・再判定は拒否する
- `game:next-responder`: 不正解の `result` から列の次の席へ回答権を移して `answering` へ。`currentSubmittedAnswer` は削除し、`lastResult` は正誤通知として保持する。オフラインの席は飛ばさない。次候補がいなければ `NO_NEXT_RESPONDER` で状態を変えない
- `game:reset`: `result` から `idle` に戻し、ラウンド、早押し列、回答者、回答テキスト、直前の結果を消す。得点と○・×数、ルールは保持する
- `tournament:finish`: `game:reset` と同じ範囲をリセットして `finished` へ
...
/** Host advances after a wrong judgement without scoring again. */
type GameNextResponderPayload = Record<string, never>;

/** Host sets the rules before the first judgement. */
type GameRules =
  | { type: "points"; correctPoints: number; wrongPoints: number }
  | { type: "maruBatsu"; correctTarget: number; wrongLimit: number };
type GameRulesUpdatePayload = { rules: GameRules };
...
- ack を使わない: `tournament:leave`、`game:buzz`、`answer:submit`、`judge:submit`、`game:reset`、`game:next-responder`、`game:rules-update`、`tournament:finish`、`room:close`
...
export type SocketErrorCode =
  ...
  | "NO_NEXT_RESPONDER"       // game:next-responder with an empty queue
...
- 新規参加は大会ステータスが `active` のときだけ許可する。`closed` は `TOURNAMENT_NOT_JOINABLE`
- 再参加は、復元済みまたはメモリ上のルームに `participantId` が存在する場合に限り、大会ステータスが `closed` でも許可する。終了後に同じ席で最終結果を見直すためであり、新しい席は作らない
...
- 新ホストを未判定の早押し列から除く。ただし `result` で既に判定された本人が引き継ぐ場合、その席のエントリは次候補を特定する履歴として保持する（回答権はない）。新ホスト本人が回答権を持っていた場合は未判定回答を破棄し、列の次の回答者へ渡す。次がいなければラウンドをクリアして `idle` へ戻す。ホストが自分自身を判定する状態を作らない
...
- `buzzSessionId` は現在のラウンドIDと一致すること。同じ回答者が後の問題で再び回答中でも、遅延した判定を拒否する
- `scoreDelta` と○・×数はルームに保存したルールからサーバーが決める。クライアントが送った増減点や次アクションは受け付けない

`game:rules-update`

- ホストのみ。`idle` かつ全員の得点・○・×数が0のときだけ受理する。旧スナップショットで既に得点がある場合もルールは固定する
- 共有バリデーションを通して既知フィールドだけを保存し、`broadcastRoomState()` で全員へ配信する

`game:next-responder`

- ホストのみ。不正解の `result` で次候補がいる場合だけ受理する。得点は変更しない
...
- `Tournaments.status` を `closed` にする更新を再度実行し、成功していることを確認する。この更新は冪等なので、`tournament:finish` で既に成功していても問題ない
- DynamoDB更新に失敗した場合は `INTERNAL_ERROR` を返し、接続とルーム所有権を維持する。ホストが再度閉じれば更新を再試行できる
- 更新成功後に全員へ `room:closed` を送り、接続を切り、`RoomSnapshots` を削除してからルームの所有権を手放す
...
- クライアントは初回接続だけでなく、Socket.io の `connect` が発生するたびに同じjoinイベントを送る
...
- クライアント: 共有ルールに基づく順位・勝ち抜け表示、早押し済み表示などの派生値の計算
...
- 操作者はクライアントが送る `participantId` から特定せず、Socketセッションから特定する。例外は、フェーズ0の `tournament:join` における再参加席の申告と、`judge:submit` における判定対象の指定であり、どちらも操作主体を表さない
...
- 当初の成果物: 判定と次アクション、スコア加算、リセット規則の純粋関数 + テスト。後述のゲーム機能改訂で、即時判定と別イベントによる進行に置き換え済み
- 成果物: 状態 × 判定・進行イベントのテスト
- 制約: ステップ9と同じく同期の純粋関数として実装する
- レビュー観点: 判定と進行のリセット範囲、次候補なしの扱い
- 確認: テストのみ

**ステップ13: ホスト進行画面**
- 成果物: `judge:submit` と `game:reset` のハンドラ、ホスト進行画面、判定UI（改訂後は正誤ボタンで即時判定し、進行は別操作）、状態別のボタン出し分け
...
**ステップ15: 再接続・多重接続と終了処理の補強**
- 成果物: 参加者IDのブラウザ保存、Socket再接続のたびの再join、再接続時の復帰、接続中の操作無効化、古い接続の無効化
- 成果物: `room:close` はDynamoDBの `closed` 更新成功後にだけ接続切断と所有権解放を行い、失敗時はルームを維持する
- レビュー観点: 同じIDで複数タブを開いた場合の挙動、スコアの維持、古いSocketのdisconnectが新しい接続をオフラインにしないこと、クローズ時のDynamoDB再確認
- 確認: リロードと一時的な通信切断のどちらでもスコアと表示名が復帰し、DynamoDB更新失敗時はルームが閉じない
...
**ステップ17: スナップショットと復旧**
- 成果物: `RoomSnapshots` リポジトリ、直列化したデバウンス書き込み、起動時または初回接続時の読み戻し、TTL設定
- 成果物: `finished` のスナップショットはルームクローズまで残し、終了済み大会でも既存参加者だけは結果へ再接続できる
- レビュー観点: 書き込み頻度、古い書き込みによる上書き防止、空ルーム作成前の復元、クローズ時の保留書き込み停止と削除順序
- 確認: **サーバーを再起動しても進行中の大会が復旧し、終了後の既存参加者も結果へ戻れる**
...
### ゲーム機能の改訂: 試合ルールと即時判定（1ステップ）

ステップ19までの実装後、インフラ実装の前に行う、ユーザー要望によるゲーム仕様の改訂。旧ステップ12・13の「点数と次アクションを判定時に選択」は上記の「試合ルールと即時判定」に置き換える。

- 成果物: 得点制・n○m×の共有型と検証、7○3×プリセット、ホストの事前設定UI
- 成果物: サーバーでの得点・○・×更新、勝ち抜け・失格者の早押し拒否、判定と進行の分離
- 成果物: 参加者への即時正誤表示、○・×表示、ルール対応の最終結果、スナップショット検証と旧形式の既定値補完
- 旧形式で○・×数がない場合は0、ルールがない場合は既定の得点制に補完する。過去の得点から○・×数を推測しない
- 確認: 純粋関数の境界・権限・二重判定テスト、実Socketで全員への配信と回答秘匿、ブラウザから正誤ボタン1回での結果表示、`npm run check`
...
- 必須の指定:
  ...
  - ECSサービスは `desiredCount: 1`、`assignPublicIp: true`、`minHealthyPercent: 0`、`maxHealthyPercent: 100`
...
- レビュー観点: NAT Gatewayが作られていないこと、デプロイ中もECSタスクが2つ並存しないこと、S3が公開されていないこと、IAMが過剰でないこと
...
フェーズ0は、招待した身内だけが参加する前提である。通常操作やインフラ動作で顕在化する不具合は対策するが、意図的な攻撃や将来拡張だけを理由に複雑な仕組みは追加しない。

- **参加者IDのなりすまし**: `participantId` はルーム状態として参加者にも見え、再参加時は秘密情報ではなく席の申告として使う。悪意ある参加者は他人のIDを指定できるため、限定公開へ移る前に秘密の再接続トークンへ置き換える
- **ホスト引き継ぎの横取り**: `paused` 中は任意のオンライン参加者が先着でホスト権限を取得できる。身内運用では許容し、限定公開前に待機時間または引き継ぎ用秘密情報を追加する
- **最大200msの状態巻き戻り**: 強制終了がデバウンス中に重なった場合、最後の状態変更がスナップショットに入らない可能性を許容する。イベントソーシング、操作ログ、変更ごとの同期書き込みは導入しない
- **RoomState型の予防的な全面改修**: status別のdiscriminated union化や、ホスト向け状態の完全なDTO分離は、現行フィールドだけでは顕在バグにならないため行わない。秘密の再接続情報など内部専用フィールドを追加する時点で再検討する
- **AppStack再作成によるオリジン変更**: CloudFrontドメインが変わると、旧オリジンのlocalStorageに保存したホストトークンへ新オリジンからアクセスできない。フェーズ0では進行中・未クローズの大会がないことを確認してからAppStackを削除する
- **CloudFront以外からのALB到達対策**: マネージドプレフィックスリスト、オリジン固有ヘッダー、オリジンHTTPSの完全な組み合わせは限定公開時に導入する
- **大規模化対策**: イベントソーシング、Redis、マルチインスタンス、差分状態配信、マルチリージョンは実測で必要になるまで導入しない
